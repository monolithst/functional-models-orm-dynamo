...
import { OrmQuery, OrmQueryStatement, PropertyStatement } from 'functional-models-orm/interfaces'
...
const _idGenerator = (existingFunc: any): ((s: any)=>string) => {
  if (!existingFunc) {
    return (_: string) => v4().replace(/-/gu, '').replace(/_/gu, '')
  }
  return existingFunc
}
...
const queryBuilder = ({ createUniqueId }:{ createUniqueId?: ((s: any)=>string)} ) => (tableName: string, queryData: OrmQuery) => {
  ...
  const realProperties : DynamoPropertyStatement[] = flowObjs.filter(_isPropertyStatement).map(x=>x as DynamoPropertyStatement)
  ...
  const propKeyToKey : {[s: string]: string}= realProperties.reduce((acc, obj) => {
    return merge(acc, {
  ...
  const propNametoExpressionAttribute : {[s: string]: string} = realProperties.reduce((acc, obj) => {
  ...
  const _combineStatementAndGetPreviousOrm = (acc: string, previous: DynamoOrmStatement|undefined, flowObj: DynamoOrmStatement) : [string, DynamoOrmStatement|undefined]=> {
      if (flowObj.type === 'property') {
        ...
        if (previous && _isPropertyStatement(flowObj)) {
          ...
          return [
            acc + `${expressionAttribute} ${_getEqualitySymbol(flowObj)} :${propKey}`,
            flowObj,
          ]
        }
        return [
  ...
  const _createFilterExpression = () : string => {
    return flowObjs.reduce(
      ([acc, previous]: [string, DynamoOrmStatement|undefined], flowObj: DynamoOrmStatement) => {
